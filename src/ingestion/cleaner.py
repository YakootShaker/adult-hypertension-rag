...
RUNNING_HEADERS: set[str] = {
    ...
    # NOTE (updated): bare "RECOMMENDATIONS" / "RECOMMENDATION" lines ARE safe
    # to strip here. The chunker's section detection relies exclusively on
    # numbered headings like "1. RECOMMENDATION ON BLOOD PRESSURE THRESHOLD..."
    # (matched by NUMBERED_REC_PATTERN), so removing the standalone word does
    # NOT break section detection — it only removes header/footer leakage.
    "RECOMMENDATIONS",
    "RECOMMENDATION",
}
